...
import { Divider, Form, Label, Button, Radio, Checkbox, Tab, Card, Image } from 'semantic-ui-react';
...
function TokenPage(props: Props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await fetch('http://127.0.0.1:3002/user-management/user/456?data=phone_number&data=email&data=given_name&data=family_name&data=preferred_username&data=two_factor');
      let data = await result.json();
      console.log(data);
  ...
  const panes = [
    {
      ...
      render: () => <Tab.Pane attached={false}>
        <Form>
          ...
          <Divider />
          <Form.Field>
            <Checkbox toggle label='TwoFactor' checked={twofactor} disabled={true} />
          </Form.Field>
        </Form>
      </Tab.Pane>,
    },
